...
`docker compose stop go`. Альтернатива — закомментировать всю секцию `go` в `docker-compose.yml`. Оба варианта предполагают запуск PHP в контейнере, а Beast локально.