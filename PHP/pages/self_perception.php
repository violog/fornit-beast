...
?>
<div style='position:absolute;top:38px;left:180px;font-family:courier;font-size:16px;cursor:pointer;'
...
<div class='navigator_button' style='position:absolute;top:38px;left:510px;cursor:pointer;' onClick="get_tree1()"
	title='Дерево текущей ситуации с произвольной активацией'>Дерево ситуации</div>

<div class='navigator_button' style='position:absolute;top:38px;left:670px;cursor:pointer;' onClick="get_tree_problem()"
	title='Дерево проблем'>Дерево проблем</div>
...
<div style='margin-top:10px;text-align:left;'>
	...
	<div class='navigator_button' style='cursor:pointer;color:red;' onClick="clian_memory_psy();"
		title='Очистить всю память психики в папке memory_psy'><b>Очистить всю память психики (папка memory_psy)</b>
	</div>
	...
	<span class="spoiler_header" onclick="open_close('lib_block1_id',1)"
		style="cursor:pointer;font-size:16px"><?= set_sopiler_icon('lib_block1_id') ?><b>Дерево эпизодической памяти,
			Правила</b></span>
	<div id="lib_block1_id" class="spoiler_block spoiler"
		style="position:relative;z-index:10;top:0px;left:0px;padding-left:15px;background-color:#ffffff;width:1100px;height:0px;">
		Дерево сохраняет Правила и Модели понимания (см. ниже спойлер Модели понимания). Здесь показываются 100
		последних Правил, записанных в кадрах эпизодической памяти, включая "учительские".<br>
		<br>
	...
	<div class='navigator_button' style='cursor:pointer;color:red;' onClick="clian_episodic();"
		title='Очистить эпизодическую память ТОЛЬКО ПОСЛЕ ВКЛЮЧЕНИЯ Beast!'><b>Очистить эпизодическую память</b></div>
	<div class='navigator_button' style='cursor:pointer;' onClick="get_rules()" title='Таблица моторынх Правил'>
		<b>Правила</b></div>
	<hr style='width:90%' align='left'>
...
<div class='navigator_button' style='cursor:pointer;' onClick="get_map_uundstd_obj()" title=''><b>Карта моделей
		понимания</b></div>
<hr style='width:90%' align='left'>


<span class="spoiler_header" onclick="open_close('lib_block11_id',1)"
	style="cursor:pointer;font-size:16px"><?= set_sopiler_icon('lib_block11_id') ?><b>Дерево ментальной эпизодической
		памяти. Ментальные правила (автоматизмы).</b></span>
<div id="lib_block11_id" class="spoiler_block spoiler"
	style="position:relative;z-index:10;top:0px;left:0px;padding-left:15px;background-color:#ffffff;width:1100px;height:0px;">
	Носитель ментальных правил: какая цепочка инфо-функций в условиях данной проблемы и темы привела к данному
	эффекту.<br>
	Выполняет роль Ментальных автоматизмов при поиске мент.правила (последователньости инфо-функций) для уверенного
	запуска в данных условиях.

	<br>
</div>
<div class='navigator_button' style='cursor:pointer;color:red;' onClick="clian_ment_episodic();"
	title='Очистить эпизодическую память ТОЛЬКО ПОСЛЕ ВКЛЮЧЕНИЯ Beast!'><b>Очистить мент. эпизодическую память</b></div>
<div class='navigator_button' style='cursor:pointer;' onClick="get_ment_rules()" title='Таблица ментальных Правил'>
	<b>Ментальные правила</b></div>
<hr style='width:90%' align='left'>




<span class="spoiler_header" onclick="open_close('lib_block2_id',1)"
	style="cursor:pointer;font-size:16px"><?= set_sopiler_icon('lib_block2_id') ?><b>Циклы осмысления</b></span>
<div id="lib_block2_id" class="spoiler_block spoiler"
	style="position:relative;z-index:10;top:0px;left:0px;padding-left:15px;background-color:#ffffff;width:1100px;height:0px;">
	Цикл осмысления - субъективный ориентировочный рефлекс, каждой шаг которого основывается на информации, даваемой
	предыдущим шагом
	с целью найти подходящие действия для данной ситуации, что дает возможность снова сориентироваться.<br>
	После каждой активации по объективному оринентировочному рефлексу возникает цикл ментальной обработки информации,
	который заключается в поиске подходящей информационной функции, запуска ментального автоматизма этой функции,
	получения новой информации, использования ее последущей инфо-функцией и так - до создания ментального автоматизма
	запуска моторного действия, которое проверяется на успешность и прописывается как моторный автоматизм.<br>
</div>
<div class='navigator_button' style='cursor:pointer;' onClick="get_cicles()"
	title='Ментальные циклы - кратковременная память'><b>Циклы</b></div>
<hr style='width:90%' align='left'>



<span class="spoiler_header" onclick="open_close('lib_block33_id',1)"
	style="cursor:pointer;font-size:16px"><?= set_sopiler_icon('lib_block33_id') ?><b>Цепочки действий (программы
		действий)</b></span>
<div id="lib_block33_id" class="spoiler_block spoiler"
	style="position:relative;z-index:10;top:0px;left:0px;padding-left:15px;background-color:#ffffff;width:1100px;height:0px;">
	Последовательность отдельных известных образов действий образует программу действий с пусковым начальным звеном.
	Такая цепочка действий, хранящаяся в оперативной памяти, может быть запущена как целая программа последовательности
	действий и тогда по оценке результата первое звено становится автоматизмом, а последующие - продолжением его
	действия.<br>Становится возможной осознанная корректировка сложных программ действия и выполнение этой программы уже
	без размышления.<br>Так же становится возможным осознанный прогноз возможных результатов этих действий.<br>
</div>
...
<span class="spoiler_header" onclick="open_close('lib_block3_id',1)"
	style="cursor:pointer;font-size:16px"><?= set_sopiler_icon('lib_block3_id') ?><b>Значимость элементов
		восприятия</b></span>
<div id="lib_block3_id" class="spoiler_block spoiler"
	style="position:relative;z-index:10;top:0px;left:0px;padding-left:15px;background-color:#ffffff;width:1100px;height:0px;">
...
<hr style='width:90%' align='left'>
<span class="spoiler_header" onclick="open_close('lib_block5_id',1)"
	style="cursor:pointer;font-size:16px"><?= set_sopiler_icon('lib_block5_id') ?><b>Доминанты</b></span>
<div id="lib_block5_id" class="spoiler_block spoiler"
	style="position:relative;z-index:10;top:0px;left:0px;padding-left:15px;background-color:#ffffff;width:1100px;height:0px;">
	Доминанта нерешенной проблемы – постоянно существующая структура, где запоминается проблема, условия и попытки ее
	решения.<br>
	Доминанта активируется во многих местах адаптивных механизмов и позволяет находить решение новым способом – по
	аналогии.<br>
	При удачном решении и его реальной проверки, доминанты закрывается (закрытие гештальта), но остается в базе опыта
	решений.<br>
	Доминанта — это просто пролонгированная Цель. Повышенное время существования Доминанты повышает шансы найти решение
	по аналогии func checkRelevantAction
	Активная Доминанта, если она есть, ВСЕ ВРЕМЯ сопровождает появление новой инфы восприятия так, чтобы была
	возможность найти решение по аналогии.<br>
	Решенная Доминанта является третьим типом формирования автоматизмов на уровне пятой ступени развития: творческое
	формирование автоматизма.<br>
